...
const ProductCard = ({ product, onAdd }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Image Container */}
        <div className="relative h-60 w-full overflow-hidden bg-[#F5F1EB] flex items-center justify-center">
          {imageUrl ? (
            <motion.img
              src={imageUrl}
              alt={product.name}
          ...
          {product.stockQuantity > 0 && (
            <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full flex items-center gap-1 shadow-sm">
              ...
              <span className="text-[11px] font-bold text-[#422701]">IN STOCK</span>
            </div>
          )}
        </div>

        {/* Content */}
        <div className="p-6 pb-7 flex flex-col flex-grow">
          <span className="text-[10px] font-bold text-[#705E46] uppercase tracking-[0.1em] mb-2.5">
            {product.categoryName || product.category || 'Organic'}
          </span>

          <h3 className="text-[20px] font-bold text-[#422701] mb-2 leading-tight line-clamp-2">
            {product.name}
          </h3>

          {product.description && (
            <p className="text-[13px] text-[#705E46] leading-relaxed mb-4 flex-grow line-clamp-2 opacity-80">
              {product.description}
            </p>
          )}
          ...
          <div className="flex items-center justify-between mt-auto pt-2">
            ...
            <motion.button
              ...
              whileHover="whileHover"
              whileTap="whileTap"
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); onAdd && onAdd(); }}
              className="w-10 h-10 rounded-xl bg-[#422701] text-[#D6B588] flex items-center justify-center hover:shadow-lg transition-shadow focus:outline-none"
